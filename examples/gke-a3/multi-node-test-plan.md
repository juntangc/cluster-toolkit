# A3 High GPU Multi-Node Test Plan (TCPX)

> [!NOTE]
> This step-by-step test plan is tailored specifically for **A3 High GPU (`a3-highgpu-8g`)** clusters utilizing GPUDirect TCPX (4-NIC multi-VPC networking) with `nccl-test.yaml` and `nccl-config.yaml`. 
> - For **A3 Mega (TCPXO)**, use `nccl-test-latest.yaml`.
> - For **A3 Ultra (RoCE / JobSet)**, use `nccl-jobset-example.yaml` or `system_benchmarks/`.
...
We use the file `examples/gke-a3/nccl-test.yaml` for a multi-node test. By default, this file contains the definitions for 2 nodes (`nccl-test-host-1` and `nccl-test-host-2`). Make the necessary changes to this YAML file if you want to use a number of nodes other than 2 (by adding additional Pods and Services).

> [!NOTE]
> **Testing > 2 Nodes:**
> If your cluster has more than 2 nodes and you wish to run the NCCL test across all of them, you must manually update `examples/gke-a3/nccl-test.yaml` by duplicating the definitions:
>
...
If you want to test nodes provisioned by Flex Start, you must add the max run duration annotation to the `metadata.annotations` section of **every** `Pod` defined in `examples/gke-a3/nccl-test.yaml`:
...
```bash
kubectl delete -f examples/gke-a3/nccl-test.yaml
```
